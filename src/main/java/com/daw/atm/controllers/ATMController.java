...
public String processarIngressar(@ModelAttribute Diners diners, @RequestParam String numeroCompte, Model model) {
    try {
        int quantitat = Integer.parseInt(diners.getQuantitat());
        if (quantitat % 10 == 0) {
            Compte compte = atm.getBanc().getCompte(numeroCompte);
            if (compte != null) {
                atm.ingressar(compte, quantitat);
                model.addAttribute("missatge", "S'ha realitzat l'ingrés");
            } else {
                model.addAttribute("missatge", "Compte no trobat.");
            }
            return "ingressar";
        } else {
